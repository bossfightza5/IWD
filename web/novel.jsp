            ...
            <div class="r_cont">
                ...
                <sql:query var="result" dataSource="${dokfah}">
                       ...
                       <div style="float: left; width:750px; text-align: center;">
                           <a href="http://localhost:8080/DokFah/novel.jsp?page=1">first page</a>
                           <% if(p<=3){
                               for(int j= Math.max(1, p-3);j<Math.min(maxpage+1, 8);j++){
                             if(j!=p)
                                out.println("<a href='http://localhost:8080/DokFah/novel.jsp?page="+j+"'>"+j+"</a>");
                             else
                                 out.println(j);
                               }
                           }else if(p>=maxpage-3)
                       ...
                       %>
                       <a href="http://localhost:8080/DokFah/novel.jsp?page=<%= maxpage %>">last page</a>
                       </div>
                       </c:when>
                        <c:otherwise>
                            <c:redirect url="./novel.jsp?page=1"/>
                        </c:otherwise>
                        </c:choose>
            </div>
        </div>
